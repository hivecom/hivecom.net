...
<template>
  ...
  <!-- Channel info modal -->
  <Modal v-if="activeBuffer?.kind === 'channel'" :open="open" size="m" @close="emit('close')">
    <template #header>
      <Flex x-between y-center gap="s">
        ...
        <template v-if="canEdit">
          <Button plain square @click="emit('close'); channelSettingsOpen = activeBuffer!.name">
    ...
    <Flex column gap="m">
      ...
      <Flex v-if="activeBuffer.metadata?.get('markdown')" column gap="xxs">
        <span class="chat-info-modal__section-label">About</span>
        <MarkdownRenderer :md="activeBuffer.metadata.get('markdown')!" skeleton-height="60px" />
      </Flex>
