// Fullscreen-player smoke visualizer, inspired by XRA's Methlab player. Emitters
// live in 3D world space but accumulate in screen-space feedback buffers, so a
// moving camera smears world-fixed points into trails.
//
// There are two buffers. The slow one fades, blurs and advects along curl noise
// so stamps dissolve into smoke. The fast one clears almost at once so hits pop
// and vanish.
//
...
// Each colour is a 0..1 [r, g, b] triple read from the VUI tokens.
...
// The feedback loop is fill-rate bound (several fullscreen passes a frame), so
// resolution past this costs a lot for little gain.
const MAX_PIXEL_RATIO = 1.25
const MAX_LONG_EDGE = 1800

// Round-robin pools. Dead points get culled in the vertex shader.
...
// Each layer composites through its own feedback buffer.
const LAYER_BG = 0
const LAYER_FG = 1

// Live-tunable from the settings panel. The engine reads these every frame.
export interface SmokeConfig {
  cameraDistance: number
  cameraDrift: number
  cameraAudioKick: number // 0 ignores the music
  cameraBeatPump: number
  cameraSway: number
  ...
  smokePersist: number // feedback decay, near 1
  turbulence: number
  buoyancy: number
  smokeBlur: number
  bedDensity: number
  ...
  bassBloomSize: number
  bassPush: number
  bigFlashSize: number
  bigPush: number
  shards: boolean
  lines: number
  lineAccent: number // 0 = all lines are cutouts, 1 = all accent
...
// Feedback pass: fade, soft blur and curl-noise advection of the prior frame.
const ADVECT_FRAG = /* glsl */ `
  ...
  // Domain-warped multi-octave curl. The warp and the finer octaves turn a
  // uniform drift into plumes and filaments instead of drifting blobs.
  vec2 flowField(vec2 p) {
    vec2 warp = vec2(
      gnNoise(p * 1.1 + uTime * 0.04),
      gnNoise(p * 1.1 + 5.2 - uTime * 0.05)
    );
  ...
  void main() {
    ...
    // uPush shoves the whole frame one way on a bass hit.
    vec2 uv = vUv + (flow + uPush) * uTexel;

    // Energy-preserving 5-tap blur: the weights sum to 1.
...
// Display pass. Each buffer holds one signed ink density (all channels equal).
// Positive slow ink tints toward the accent, positive fast ink toward the text
// colour, negative ink toward the darker background. Vignette and grain only
// touch the ink so the empty field is exactly the theme background.
const DISPLAY_FRAG = /* glsl */ `
  ...
  void main() {
    ...
    // Time is floored to 24 steps a second so the dust twinkles rather than boils.
...
// fbm noise carves each stamp into a torn splotch. The per-point seed keeps any
// two from looking alike.
...
// Both modes use the same shader and a positive brightness. The blend mode
// decides the sign.
class SplotchLayer {
  ...
  constructor(t: typeof THREE, private readonly count: number, mode: 'add' | 'sub', layer = 0) {
    ...
    if (mode === 'add') {
      ...
      // dst - src, so the stamp carves a hole where smoke already is.
...
// Keyed on the rounded base position so every copy of a shared vertex gets the
// same shove. Otherwise a shard tears apart at the seams.
...
// Distorted wireframe polyhedra flashed in on the heaviest kicks.
class ShardLayer {
  readonly group: THREE.Group
  private readonly meshes: THREE.Mesh[] = []
  private readonly mats: THREE.MeshBasicMaterial[] = []

  // Undistorted base positions, so each hit warps from the clean shape.
  private readonly bases: Float32Array[] = []

  // 1 on spawn, down to 0.
  ...
  // Brightness is life squared so it snaps in hard then eases off.
  update(step: number) {
    for (let i = 0; i < this.count; i++) {
      if (this.life[i]! <= 0)
        continue
...
// Streaks lit for a single frame on a sharp hit. Per-vertex colour lets one
// segment light while the rest stay dark.
class LineLayer {
  ...
  // Returns whether the colour buffer changed.
  clearLit(): boolean {
    let dirty = false
    for (let i = 0; i < this.count; i++) {
      if (this.lit[i]) {
  ...
  // Anchored at `a`, running along `b` by `half` in both directions.
...
export class SmokeField {
  ...
  // Advances by pace rather than real time, so motion follows the song.
  private flowTime = 0
  ...
  // Rising-edge latches, so each hit fires once instead of every frame the
  // feature sits above threshold.
  ...
  // The wrap back to 0 marks a downbeat.
  private prevBeatPhase = 0

  // 0..1, peaks on the downbeat.
  private beatPulse = 0

  // EMA of the raw level. The band features are adaptively normalized, so they
  // can't tell a quiet passage from a loud one.
  private drive = 0

  // Leaky peak hold, so the gate keys on loud versus quiet within this track.
  private loudRef = 0
  private readonly scratch: THREE.Vector3
  ...
  private constructor(t: typeof THREE, canvas: HTMLCanvasElement, colors: SmokeColors, config: SmokeConfig) {
    ...
    // Streak lines run ~20k units. The targets have no depth buffer, so the deep
    // far plane costs no precision.
  ...
  resize(width: number, height: number, dpr: number) {
    ...
    // Over-spread by 1.3 so mid-radius particles reach the horizontal edges, not
    // just the sparse outermost ones.
  ...
  frame(f: AudioFeatures, dt: number, time: number) {
    ...
    // Raised cosine of the beat phase, so it stays continuous across the wrap.
    const beatTarget = f.bpm > 0 ? 0.5 + 0.5 * Math.cos(f.beatPhase * Math.PI * 2) : 0
    this.beatPulse += (beatTarget - this.beatPulse) * 0.3

    // BPM normalized around 120. Pace floors near 0 so a silent field settles
    // instead of swirling forever.
    ...
    // Dynamics gate. Turbulence below thresholds on it, so it goes first. Fast
    // attack so a drop opens up right away, slow release so it settles.
    this.drive += (f.level - this.drive) * (f.level > this.drive ? 0.2 : 0.05)

    // Relative to this track's range, since level's log scale rarely climbs near 1.
    ...
    // quiet raises every hit threshold and gate shrinks what slips through.
    const quiet = 1 - gate

    // 1 in quiet passages, 0 once the mix gets loud. Dust only belongs to the calm parts.
    const lowFactor = Math.max(0, 1 - gate * 1.8)

    // Turbulence stays calm until gate > 0.9, so only drops churn the smoke.
    ...
    // A touch quicker when busy so hits don't pile up.
    ...
    // Adaptive bass trigger. In a dense mix the kick barely lifts the spectrum, so
    // no fixed threshold works. The baseline falls fast and rises slow so kick peaks
    // don't drag it up, and a kick fires when it clears half the recent peak height.
    // Calibrates to a track in ~1-2s. The band energy check stops a phantom frequency
    // from tripping it.
    ...
    if (this.bassArmed && f.bassHit > fireLevel && bassEnergy > 0.15) {
      ...
      // Relative too, so shards punctuate drops rather than every bar.
    ...
    // The band energy check stops a track with no top end from false-firing.
    ...
    // A burst on each downbeat keeps the field breathing through stretches with no
    // sharp transient.
    if (f.bpm > 0) {
    ...
    // A slow brush that keeps stamping, so calm passages draw continuous lines
    ...
    // Busier passages fade faster so the screen clears.
    ...
    if (lineHit && !this.lineHeld && Math.random() < (0.4 + f.geometry * 0.5) * gate * this.cfg.lines) {
      // Endpoints land far off screen so a streak never ends mid-frame at any
    ...
    // Moving the world, more than the screen swirl, is what drags the smoke.
    ...
    // Idle drift only. The audio kick lands in the bass trigger so camera and
    // shards move together.
    ...
    // Gated by cameraAudioKick so at 0 the distance holds steady.
    this.radius += ((this.cfg.cameraDistance - f.energy * 1.2 * this.cfg.cameraAudioKick) - this.radius) * 0.02

    // Applied on top of the base radius, which lerps far too slowly to track a beat.
  ...
  private render() {
    ...
    // Background smoke: full blur and swirl, then the background layer on top.
    ...
    // Foreground hits: barely any blur or swirl.
