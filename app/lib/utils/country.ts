...
export interface CountryInfo extends CountryDefinition {
  emoji: string
}
...
function computeFlagEmoji(code: string): string {
  const base = 0x1F1E6
...
export function getCountryName(code: string | null | undefined): string | null {
  ...
  return COUNTRY_MAP.get(normalized)?.name ?? null
}
...
export function getCountryInfo(code: string | null | undefined): CountryInfo | null {
  ...
  const country = COUNTRY_MAP.get(normalized)
  if (country == null)
    return null

  return {
    ...country,
    emoji: computeFlagEmoji(normalized),
  }
}
...
export const COUNTRY_SELECT_OPTIONS = COUNTRIES.map((country) => {
  ...
  return {
    label: `${emoji} ${country.name} (${country.code})`,
    value: country.code,
    emoji,
  }
})
