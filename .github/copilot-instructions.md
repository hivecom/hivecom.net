...
- Tiptap mention trailing space: `@tiptap/extension-mention`'s suggestion `command` always appends a trailing space text node after the inserted mention atom. This means the cursor after accepting a suggestion sits **after the space**, not directly after the mention node. Any keyboard-shortcut logic that checks `$from.nodeBefore` for a mention will miss it - you must also check whether `nodeBefore` is a pure-whitespace text node and then look one step further back for the mention. See `app/components/Editor/plugins/mentions.ts` for the reference implementation.
