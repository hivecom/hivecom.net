DROP FUNCTION IF EXISTS public.get_admin_users_paginated(text, text, text, text, text, text, text, integer, integer);
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE
      ...
      -- provider filter (auth provider): google, discord, email
      AND (
        p_provider = ''
        OR u.raw_app_meta_data->'providers' @> to_jsonb(p_provider::text)
      )
      -- platform filter: steam, discord, patreon, teamspeak
      AND (
        p_platform = ''
        OR (p_platform = 'steam'     AND p.steam_id IS NOT NULL)
        OR (p_platform = 'discord'   AND p.discord_id IS NOT NULL)
        OR (p_platform = 'patreon'   AND p.patreon_id IS NOT NULL)
        OR (p_platform = 'teamspeak' AND p.teamspeak_identities IS NOT NULL AND jsonb_array_length(p.teamspeak_identities) > 0)
      )
      -- supporter filter: 'true' = is supporter, 'false' = not supporter
      AND (
        p_supporter = ''
        OR (p_supporter = 'true'  AND (COALESCE(p.supporter_lifetime, false) OR COALESCE(p.supporter_patreon, false)))
        OR (p_supporter = 'false' AND NOT (COALESCE(p.supporter_lifetime, false) OR COALESCE(p.supporter_patreon, false)))
      )
