-- Fix get_admin_discussions_paginated (with p_author_id) to include theme_id,
-- theme_name, the themes JOIN, and the 'themes' context filter - mirroring the
-- no-author-filter overload which already had these fields.
--
-- Root cause: when the p_author_id overload was introduced in
-- 20260414220857_admin_discussions_paginated_author_filter.sql it was based on
-- the pre-theme version of the function. The subsequent theme additions in
-- 20260417165620_admin_discussions_rpc_add_theme_context.sql only updated the
-- overload without p_author_id, leaving the active overload incomplete.

DROP FUNCTION IF EXISTS public.get_admin_discussions_paginated(text, text[], text[], uuid, text, text, integer, integer);
