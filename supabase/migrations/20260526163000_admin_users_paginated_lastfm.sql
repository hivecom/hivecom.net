-- =============================================================================
-- admin_users_paginated_lastfm
-- =============================================================================
-- Adds lastfm_username to the get_admin_users_paginated RPC return set.
-- The column was added in 20260519220925_lastfm_integration but was not
-- included in the function definition at that time.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. Drop existing function (all signatures)
-- ---------------------------------------------------------------------------

DROP FUNCTION IF EXISTS public.get_admin_users_paginated(text, text, text, text, text, text, text, text, text, integer, integer) CASCADE;

-- ---------------------------------------------------------------------------
-- 2. Recreate with lastfm_username
-- ---------------------------------------------------------------------------
        ...
        WITH base AS (
            SELECT
                ...
                (
                    ...
                    (CASE WHEN p.lastfm_username IS NOT NULL THEN 1 ELSE 0 END)
                )                                                             AS platform_count,
            ...
            WHERE
                ...
                AND (
                    ...
                    OR ($5 = 'lastfm'    AND p.lastfm_username IS NOT NULL)
                )
                AND (
                    $6 = ''
...
-- ---------------------------------------------------------------------------
-- 3. Restrict access (mirror existing security pattern)
-- ---------------------------------------------------------------------------

REVOKE EXECUTE ON FUNCTION public.get_admin_users_paginated FROM anon, authenticated;
