-- Add p_game_ids filter to get_admin_events_paginated.
-- Uses the && (overlap) operator so any matching game triggers inclusion.

DROP FUNCTION IF EXISTS public.get_admin_events_paginated(text, text, text, integer, integer, boolean, boolean);
