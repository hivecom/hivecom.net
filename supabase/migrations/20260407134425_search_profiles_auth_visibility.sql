-- Allow authenticated users to find all non-banned profiles in search_global,
-- regardless of the profile's public flag. Anonymous users still only see
-- profiles where public = true.
--
-- The function is SECURITY INVOKER so auth.uid() resolves to the calling
-- user's JWT subject - non-null when the request carries a valid session.
