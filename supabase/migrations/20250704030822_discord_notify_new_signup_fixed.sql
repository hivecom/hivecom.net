-- Migration: Recreate Discord notification function for new user signups (removes 'Hivecom Signup Bot' footer)
-- This migration drops and recreates the notify_discord_new_signup function without the custom footer
DO $$
BEGIN
  ...
  IF EXISTS(
    ...
    FROM
      pg_proc
    WHERE
      proname = 'notify_discord_new_signup') THEN
  EXECUTE 'DROP FUNCTION IF EXISTS notify_discord_new_signup() CASCADE';
END IF;
END
$$;

-- Recreate the function without the Hivecom Signup Bot footer
