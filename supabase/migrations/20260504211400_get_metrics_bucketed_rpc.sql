-- RPC: get_metrics_bucketed
-- Buckets metrics rows into equal-width intervals and averages/sums metric fields.
-- Gaps in the series are filled with NULLs for all metric columns.
...
AS $$
  ...
  bucketed AS (
    SELECT
      ...
      -- Scalar averages (snapshot) and sums (incremental deltas)
  ...
  -- -------------------------------------------------------------------------
  -- 2. Expand teamspeak.byServer keys and compute per-(bucket, key) averages
  -- -------------------------------------------------------------------------
  ts_expanded AS (
    SELECT
      b.bucket,
      kv.key,
      AVG(kv.value::numeric) AS avg_val
    FROM bucketed b,
  ...
  ts_aggregated AS (
    SELECT
      bucket,
      jsonb_object_agg(key, ROUND(avg_val)::int) AS ts_by_server
    FROM ts_expanded
    GROUP BY bucket
  ),

  -- -------------------------------------------------------------------------
  -- 3. Expand gameservers.byServer keys, drill into data.players, average
  -- -------------------------------------------------------------------------
  gs_expanded AS (
    SELECT
      b.bucket,
      kv.key,
      AVG((kv.value -> 'data' ->> 'players')::numeric) AS avg_players
    FROM bucketed b,
  ...
  gs_aggregated AS (
    SELECT
      bucket,
      jsonb_object_agg(key, ROUND(avg_players)::int) AS gs_by_server
    FROM gs_expanded
    GROUP BY bucket
  ),
  ...
  scalar_agg AS (
    SELECT
      bucket,
      ROUND(AVG(members_online))::int     AS members_online,
      ROUND(AVG(members_total))::int      AS members_total,
      ROUND(AVG(teamspeak_online))::int   AS teamspeak_online,
      ROUND(AVG(gameservers_players))::int AS gameservers_players,
