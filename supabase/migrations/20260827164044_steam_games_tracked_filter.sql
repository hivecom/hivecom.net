-- Add a tracked filter to the admin Steam games listing. p_tracked = true
-- returns only games that exist in public.games, false only untracked ones,
-- NULL keeps the previous behavior (no filter).

DROP FUNCTION IF EXISTS public.get_admin_steam_games_paginated(text, text, text, integer, integer);
