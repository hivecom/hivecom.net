-- Fix discussion_reply_reply notifications to carry the new reply's ID as
-- source_id so clicking the notification navigates to the reply made to you,
-- not your own original comment.
--
-- Previous behaviour: source_id = reply_to_id (the original reply).
-- New behaviour:      source_id = NEW.id      (the new incoming reply).
--
-- Dedup on (user_id, source, reply_to_id) is removed; each reply-to-reply
-- now produces its own notification row, which is correct UX - you want to
-- see every reply, not just the latest one collapsed into one.
...
BEGIN
  ...
  -- source_id = NEW.id so the front-end can navigate to ?comment=<new reply>
