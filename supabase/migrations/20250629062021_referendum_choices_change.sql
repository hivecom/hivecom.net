...
BEGIN
  ...
  IF ARRAY_LENGTH(OLD.choices, 1) > ARRAY_LENGTH(NEW.choices, 1) OR NOT(OLD.choices <@ NEW.choices) THEN
    -- Check if all old choices are still present in new choices
...
COMMENT ON TRIGGER referendum_choices_removal_trigger ON referendums IS 'Automatically deletes all votes for a referendum when choices are removed (but allows adding new choices) to maintain data integrity';

COMMENT ON FUNCTION delete_votes_on_choices_removal() IS 'Function that deletes referendum votes when choices are removed. Allows adding new choices without affecting votes.';
