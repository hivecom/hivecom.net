-- Fix: get_admin_discussions_paginated used an INNER JOIN on profiles for created_by,
-- which silently dropped all discussions where created_by is NULL or the profile has
-- been deleted/scrubbed. Changed to LEFT JOIN so all discussions are always included.
