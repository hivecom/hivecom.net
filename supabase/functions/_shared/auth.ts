...
export async function authorizeSystemCron(req: Request, client: SupabaseClient<Database>) : Promise<Response | undefined> {
  ...
  if (vaultError || !vaultData) {
    throw new Error('Failed to retrieve system_cron_secret from vault: ' + (vaultError?.message || 'No data returned'))
  }
