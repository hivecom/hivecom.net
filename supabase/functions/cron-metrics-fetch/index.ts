...
// IRC stats
// ------------------------------------------------------------------------
// One authenticated stats endpoint covers users, every non-purged channel and a
// message count. Its default window matches this cron's 5 minute cadence. An
// unreachable endpoint degrades to a zeroed irc block.
//
// The metrics table and latest.json are public, so secret channels must never
// appear there by name. They're keyed by the DB's sha256 of the lowercased name,
// stored in metrics_admin_irc_channels (metrics_admin.read only). A client that
// already knows a name can find its channel without the server handing it out.
...
// A channel only counts as public when the sampler says `secret: false`. On any
// failure the secret channels are left out of the map, so the caller drops them
...
async function resolveIrcChannelKeys(
  ...
  for (const row of data ?? []) {
    if (row.secret) keys.set(row.name, row.key);
  }
...
Deno.serve(async (req: Request) => {
  try {
    ...
    const [
      ...
      // Previous snapshot, for the storage deltas
      supabaseClient
        .from("metrics")
        .select("data")
    ...
    // Opt-in users only, keyed by Steam app ID
    ...
    // Every gameserver gets an entry, even unqueryable ones
    ...
    if (queryableGameservers.length > 0) {
      const queryResults = await Promise.allSettled(
        queryableGameservers.map(async (gs) => {
          ...
          // Factorio's RCON password and Trackmania's User password live in Vault behind
          // a service_role-only RPC. They travel in the X-Query-Options header so they
          // never land in URLs or access logs.
