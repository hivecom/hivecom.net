...
// secret ones are keyed in the snapshot by that row's `key`, a sha256 of the
// lowercased name generated by the DB. Deriving the key from the name rather
// than assigning a random one lets a client that already knows the name pick
// its own channels out of the public data without anything being handed out
// server-side.
...
// key by name, secret ones by their lookup row's derived key. A channel only
// counts as public when the sampler says `secret: false`. On any failure the
