...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      try {
        ...
        if (errorData.error === 'invalid_grant') {
          errorMessage = 'Authorization code is invalid or expired. Please try connecting your Patreon account again.';
        } else if (errorData.error === 'invalid_client') {
          errorMessage = 'Client authentication failed. Please contact support.';
    ...
    const patreonUserResponse = await fetch('https://www.patreon.com/api/oauth2/v2/identity?include=memberships,memberships.currently_entitled_tiers&fields[user]=full_name,email,is_email_verified', {
      headers: {
    ...
    console.log(`Patreon account ${patreonId} connected successfully for user ${user.id}`)
