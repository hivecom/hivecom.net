...
|---|---------|--------|----------|
| 52 | `lib/discussionBadge.ts` + `lib/partyAnimalBadge.ts` + `components/Profile/Badges/ProfileBadge.vue` + `ProfileBadgeYears.vue` + `Profile/ProfileBadges.vue` + `Shared/UserPreviewCardBadges.vue` | **`BadgeVariant` type re-declared in 6 places** - all six independently declare `export/local type BadgeVariant = 'shiny' \| 'gold' \| 'silver' \| 'bronze'`. Should be in a single `lib/badgeVariant.ts` (or `lib/badges.ts`) and re-exported everywhere | High |
| 53 | `lib/partyAnimalBadge.ts` | **Inconsistent constant name**: `PARTY_ANIMAL_BADGETHRESHOLDS` (missing underscore between `BADGE` and `THRESHOLDS`) - should be `PARTY_ANIMAL_BADGE_THRESHOLDS` to match `DISCUSSION_STARTER_THRESHOLDS` / `DISCUSSION_REPLY_THRESHOLDS`. The file itself even has a `TODO` acknowledging the shared-logic problem | High |
| 54 | `lib/discussionBadge.ts` + `lib/partyAnimalBadge.ts` | **Could be merged** into a single `lib/badgeThresholds.ts` - the two files handle identical patterns (threshold constants + a `getVariant()` function) for different badge types. Unifying reduces the import surface and keeps badge logic in one place | High |
| 55 | `composables/useCacheBadgeDiscussionReplyCount.ts` `composables/useCacheBadgeDiscussionStartedCount.ts` `composables/useCacheBadgePartyAnimalCount.ts` | **383 lines of nearly identical boilerplate** - all three composables are structurally identical (same `options` interface, same `loadCount/invalidate/refresh/watch` scaffold) differing only in the Supabase query. Should be refactored to a generic `useCacheBadgeCount(config: BadgeCountConfig)` factory | High |
| 56 | `lib/cmsAssets.ts` | **Entirely dead - zero callers** - no file in the codebase imports from `@/lib/cmsAssets`. All callers have already migrated to `storageAssets.ts`. Safe to delete | High |
| 57 | `lib/user.ts` | **`useCurrentUserId` is dead code** - the function is defined but never imported or called anywhere outside the file itself. Only `extractSupabaseUserId` is used (by `composables/useUserId.ts`). Either remove or consolidate into `useUserId` | Medium |
| 58 | `lib/user.ts` line 22 | **`as never` cast** (`const id = user.id as never`) - an unsound cast used to bypass a type error, more dangerous than `as any`. Should be properly typed | Medium |
| 59 | `components/Admin/KVStore/KVStoreTable.vue` lines 2 & 9 | **Two relative `'../../../lib/kvstore'` imports** in the same file - the sole offender in the entire codebase for `../../..`-depth relative paths. Both the `import type` and value import should use `@/lib/kvstore` | Medium |
| 60 | `lib/anonymous-usernames.ts` + `lib/markdown-processors.ts` | **Kebab-case filenames** inconsistent with all other lib files which use camelCase (`banStatus.ts`, `lastSeen.ts`, `mediaQuery.ts`, etc.). Should be `anonymousUsernames.ts` and `markdownProcessors.ts` respectively. Both have active callers so all imports would need updating | Medium |
| 61 | `lib/utils/random.ts` | **Missing `shuffleArray` export** - the file provides seeded random helpers but no general-purpose Fisher-Yates shuffle. At least four separate locations implement their own shuffle inline: `BulkAvatarDisplay.vue` and `BulkAvatarDisplayCluster.vue` (byte-for-byte identical Fisher-Yates blocks), `LandingMotd.vue` (a named local `shuffle()` function), and `pages/community/index.vue` (uses the biased `arr.map(x => ({ x, sort: Math.random() })).sort()` anti-pattern twice for project shuffling). Add a single `shuffleArray<T>(arr: T[]): T[]` to `lib/utils/random.ts` and replace all four callers. | Medium |
| 62 | `lib/anonymous-usernames.ts` line 1 | **Relative path import** for the JSON file (`../../usernames.json`) - the only relative-path import in any `lib/` file. Consider moving `usernames.json` into `app/assets/` or using a path alias | Low |
| 63 | `composables/useTableActions.ts` | **Misleading name** - the composable only determines *permissions* for table actions, not the actions themselves. `useTablePermissions` would be more accurate. Also: the `catch` block silently swallows *all* errors, not just "not in admin context" ones - should check the error type before swallowing | Medium |
| 64 | `composables/useCacheAvatar.ts` | **Orphaned composable** - `useCacheAvatar` is exported but never imported anywhere in the app (all avatar fetching goes through `useCacheUserData`). Candidate for removal | Medium |
| 65 | `components/Profile/Activity/shared.ts` | **Unused** - `ActivityComponentProps` is defined but never imported (checked across all `.vue` and `.ts` files). Candidate for removal, or the `small` prop should be explicitly re-added to the Activity components that need it | Medium |
| 66 | `lib/placeholderBannerProjects.ts` | **Naming**: verbose and inverted - describes what it does (`placeholder banner`) and the subject (`Projects`), but the convention for lib files is noun-first. Consider `projectBannerPlaceholder.ts` or `projectBanners.ts` | Low |
| 67 | `lib/user.ts` | **Scope too narrow** - a 25-line file with one live export (`extractSupabaseUserId`) used by only one composable. Should be folded into `composables/useUserId.ts` to eliminate an indirection layer | Low |
...
| # | File(s) | Issue | Priority |
|---|---------|--------|----------|
| 70 | **~30+ component files** | **`Tables`, `TablesInsert`, `TablesUpdate` imported from `@/types/database.types`** instead of `@/types/database.overrides` as required by project guidelines. This is a codemod-friendly sweep (97 total files import from `database.types`; those pulling the table helpers need redirecting). Confirmed violators include: `components/Settings/ConnectionsCard.vue`, `ConnectTeamSpeak.vue`, `ProfileSummaryCard.vue`, all 6 `GameServers/` components, `Admin/Complaints/ComplaintList.vue`, `ComplaintCard.vue`, `ComplaintDetails.vue`, `Admin/Discussions/DiscussionTable.vue`, `DiscussionDetails.vue`, `Admin/Projects/ProjectTable.vue`, and more | High |
| 71 | `pages/community/funding.vue` + `components/Community/FundingProgress.vue` + `components/Community/SupportCTA.vue` | **Redundant parallel fetches of `monthly_funding`** - `funding.vue` fetches all `monthly_funding` rows for `FundingHistory`, then renders `<FundingProgress>` which independently fetches the latest row again, and `<SupportCTA>` (when rendered without props) makes a third fetch for the same latest row. All three fire on page load with no shared cache. A `useFundingData()` composable (or extending `useCache`) should own the fetch and be shared across all three, eliminating the duplicate network requests. | High |
...
| # | File | Issue | Priority |
|---|------|--------|----------|
| 72 | `components/Community/DonationBar..vue` | **Typo in filename** (double dot) + **hardcoded stub** - contains `const funding = 67` and a hardcoded username array with no props, no Supabase calls. Zero callers anywhere. Either finish it (rename to `DonationBar.vue`, add props) or delete it | High (bug) |
| 73 | `components/Community/ProjectMetadata.vue` | **Zero callers** - not imported or referenced anywhere. Nuxt auto-import would expose it as `<CommunityProjectMetadata>` but no template uses it. Candidate for deletion or integration into `pages/community/projects/[id].vue` | Medium |
| 74 | `components/Profile/Activity/ActivitySpotify.vue` | **Permanently commented out** - the only reference in `ProfileActivity.vue` is `<!-- <ActivitySpotify /> -->`. The component file (28 lines) exists but is never rendered. Either remove it or add a comment explaining why it is kept | Low |
...
| 114 | `components/Admin/Network/GameServerTable.vue` | **543 lines** | Part of the 6-file admin table boilerplate pattern - see note below | Low |
| 115 | `components/Admin/Funding/ExpenseTable.vue` | **488 lines** | Part of the 6-file admin table boilerplate pattern. Also contains standalone utility functions (`formatDate`, `calculateDuration`, `isPlannedExpense`, `getExpenseStatus`) that belong in `lib/utils/expenses.ts` | Low |
...
| 117 | `components/Admin/Games/GameTable.vue` | **453 lines** | Part of the 6-file admin table boilerplate pattern. Additionally normalizes shorthand inline in `handleGameSave` - that transform should live in `lib/games.ts` | Low |
| 118 | `components/Admin/Referendums/ReferendumTable.vue` | **528 lines** | Part of the 6-file admin table boilerplate pattern. `getReferendumStatus` and `getVoteCount` are pure functions that belong in `lib/referendums.ts` | Low |
| 119 | `components/Admin/Projects/ProjectTable.vue` | **525 lines** | Part of the 6-file admin table boilerplate pattern | Low |
| 120 | `components/Admin/Network/ServerTable.vue` | **479 lines** | Part of the 6-file admin table boilerplate pattern. `console.warn` calls remain from debugging in `handleServerSave` and `handleServerDelete` | Low |
...
| 124 | `pages/admin/network.vue` + `pages/admin/users.vue` | **Duplicated permission-gated tab management scaffold** - both pages implement the same ~40-line pattern: `availableTabs` computed from permissions, `activeTab` ref, a `watch([availableTabs, route.query.tab])` that picks the first allowed tab or respects a valid query param, and a `watch(activeTab)` that pushes to `router.query.tab`. The query-string normalization (`typeof q === 'string' ? q : Array.isArray(q) && q[0] ? q[0] : ''`) is copy-pasted verbatim in both files. Extract a `useAdminTabs(tabs: ComputedRef<Tab[]>)` composable returning `{ activeTab, syncedTab }` that handles all routing and query-param sync internally. | Medium |

### Note: The 6-file Admin Table Scaffold (items 109-115)
...
|---|----------|---------------|----------------|----------|
| 125 | `getReferendumStatus(referendum)` | `Admin/Referendums/ReferendumDetails.vue`, `Admin/Referendums/ReferendumTable.vue`, `pages/votes/index.vue` - **identical 8-line implementation in all three** | `lib/referendums.ts` (new file) | High |
| 126 | `calculateDuration(startDate, endDate?)` | `Admin/Funding/ExpenseTable.vue` and `Admin/Funding/ExpenseDetails.vue` - **byte-for-byte identical** ~20-line date-math function | `lib/utils/duration.ts` (already exists - add overload accepting string dates) | High |
| 127 | `getLastSeenVariant(status)` | `Admin/Users/UserTable.vue` and `Admin/Users/UserDetails.vue` - **identical** mapping of `UserActivityStatus` to a color-variant string (`'online' \| 'fresh' \| 'light' \| 'lighter' \| 'lightest'`) | `lib/lastSeen.ts` (already exists - append the helper) | High |
| 128 | `formatDateOnly(date: Date): string` | `Admin/Users/UserForm.vue` and `Profile/ProfileForm.vue` - **identical** `YYYY-MM-DD` formatter using `getFullYear/getMonth/getDate` | `lib/utils/date.ts` (already exists - append) | Medium |
| 129 | `formatDate(dateString)` (short locale string) | `Admin/Funding/ExpenseTable.vue`, `Community/ExpenseCard.vue`, `Shared/ComplaintsViewer.vue`, `Settings/ProfileSummaryCard.vue`, `pages/legal/[...name].vue` - five separate implementations of `new Date(x).toLocaleDateString(...)`, each with slightly different locale options | Rationalise into `lib/utils/date.ts` with named variants (`formatDateShort`, `formatDateLong`, `formatDateWithTime`) and replace all five callers | Medium |
| 130 | `isPlannedExpense(startDate)` | `Admin/Funding/ExpenseTable.vue` and `Community/ExpenseCard.vue` (as a computed) - same "start date is in the future" predicate | `lib/utils/expenses.ts` (new file, pairs with item 110) | Medium |

### Functions that belong in `lib/` but were never extracted

| # | Function / Logic | Currently in | Should move to | Priority |
|---|-----------------|--------------|----------------|----------|
| 131 | `isBanActive()` local function in `ProfileBanStatus.vue` | `components/Profile/ProfileBanStatus.vue` lines 14-26 - reimplements the same logic as `lib/banStatus.ts:isBanActive()` but reads from `props` directly instead of calling the shared function | Delete the local function and call `isBanActive(props.profile.banned, props.profile.ban_end)` from `lib/banStatus.ts` | High |
| 132 | `formatPermissionName`, `formatCategoryName`, `getRoleColor`, `getRoleVariant`, `getCategoryIcon` | All five live in `Admin/Roles/RolesTable.vue` (lines 104-187) with no callers elsewhere - pure transform functions with no Vue dependency | `lib/rolePermissions.ts` (new file) | Medium |
| 133 | `getReferendumStatus` badge-variant mapping (`'active' → 'success'`, `'upcoming' → 'warning'`, `'concluded' → 'neutral'`) | Inlined as ternary expressions in `ReferendumTable.vue` template and `ReferendumDetails.vue` template - duplicated presentation logic | `lib/referendums.ts` alongside item 118 as `getReferendumStatusVariant(status)` | Medium |
| 134 | `getVoteCount(referendum)` | `Admin/Referendums/ReferendumTable.vue` line 109 - one-liner reading `referendum.vote_count?.[0]?.count \|\| 0` | `lib/referendums.ts` alongside item 118 | Low |
| 135 | `normalizeProjectId`, `buildProjectBannerPath`, `dispatchProjectBannerUpdated` | Currently in `lib/storage.ts` mixed in with avatar/game-asset functions - the project-banner helpers are a distinct subdomain buried in a 660-line file | Extract to `lib/projectBanner.ts` (or the renamed `lib/projectBanners.ts` from item 62) to make `lib/storage.ts` easier to navigate | Low |

### `lib/utils/` gaps - missing utilities that components are re-implementing

| # | Issue | Evidence | Priority |
|---|-------|----------|----------|
| 136 | `lib/utils/date.ts` exports `formatDate(dateStr)` via dayjs but **components are not importing it** - they call `toLocaleDateString` directly. The lib function and the inline ones produce different format strings, so there is no single canonical date format for the app | 132 separate inline `formatDate` functions found (item 122); `lib/utils/date.ts:formatDate` is effectively unused outside funding charts | Medium |
| 137 | `lib/utils/duration.ts` has `formatDurationFromMinutes(minutes)` but `Admin/Funding/ExpenseTable.vue` and `Admin/Funding/ExpenseDetails.vue` both reimplement a days/months/years prose formatter (`calculateDuration`) that is **not** in `lib/utils/duration.ts`. The lib only covers ms/minutes → compact badge strings; it needs a `formatDurationBetweenDates(start, end?)` overload | Items 119 and 130 are the same root cause | Medium |
| 138 | `lib/utils/common.ts` has `getCSSVariable(key)` but `lib/charts.ts` calls it while passing a manually constructed theme-prefixed key (`--${theme}-color-border-weak`). This suggests chart theming logic should be a named helper in `lib/charts.ts` rather than a raw CSS variable lookup, since the theme-prefix convention is not documented | Low |
| 139 | `lib/utils/common.ts` | **Missing `getRouteQueryString(value)` helper** - eight call sites across `pages/admin/network.vue`, `pages/admin/users.vue`, `pages/auth/sign-in.vue`, `pages/auth/oauth/consent.vue`, and `pages/auth/callback/patreon.vue` all independently inline the same ternary: `typeof q === 'string' ? q : Array.isArray(q) && q[0] ? q[0] : ''` (or a `null`-returning variant). This belongs in `lib/utils/common.ts` as `getRouteQueryString(value: LocationQueryValue | LocationQueryValue[]): string` and `getRouteQueryStringOrNull(...)` returning `string | null`. | Low |
| 140 | `pages/events/[id].vue` + `components/Events/RSVPButton.vue` + `components/Events/EventRSVPCount.vue` + `components/Events/EventRSVPModal.vue` | **Event timing logic duplicated across four files** - all four independently declare `eventStart`, `eventEnd` (with the same `duration_minutes * 60 * 1000` offset), and `hasEventEnded` computed from a `now` ref ticked by `useIntervalFn` (or a manual `setInterval` in the page). `events/[id].vue` additionally reimplements `isUpcoming`, `isOngoing`, `timeAgo`, and `updateTime` from scratch rather than passing them down. Extract a `useEventTiming(event: MaybeRefOrGetter<Tables<'events'>>)` composable returning `{ eventStart, eventEnd, isUpcoming, isOngoing, hasEventEnded, countdown, timeAgo }` and replace all four callers. | High |
...
|---|-----------|-----------------|-------------|-------|----------|
| 141 | `'rsvp-updated'` | `components/Events/RSVPButton.vue` (×2) | `EventRSVPCount.vue`, `EventRSVPModal.vue`, `EventHeader.vue` | Four files share one magic string with no shared type for the `detail` payload (`{ eventId, newStatus }`). A `useRsvpBus()` composable wrapping a typed `EventTarget` or Mitt emitter would make the contract explicit and testable. | High |
| 142 | `'avatar-updated'` | `lib/storage.ts` (×2 - `uploadUserAvatar` and `deleteUserAvatar`) | `components/Profile/ProfileDetail.vue` (also listens on `window.storage` for the same key) | Dispatched from a lib function - not a Vue composable - so it bypasses Vue's reactivity entirely. `ProfileDetail.vue` double-listens: once via `window.addEventListener('avatar-updated', ...)` and once via `window.addEventListener('storage', event => event.key?.includes('avatar-updated'))`. The two listeners are redundant and neither is typed. Should be replaced with `invalidateUser()` from `useCacheUserData` called directly after upload. | High |
| 143 | `'project-banner-updated'` | `lib/storage.ts:dispatchProjectBannerUpdated()` | `composables/useCacheProjectBanner.ts` | Better than `avatar-updated` (has a dedicated dispatch helper), but still untyped and crosses the lib/composable boundary via a global event. The composable already has a `invalidate()` function; the dispatch could be moved entirely into the composable layer and the storage function could accept an optional callback instead. | Medium |
...
| # | File(s) | Issue | Priority |
|---|---------|-------|----------|
| 144 | `components/Discussions/Discussion.vue` (10 `provide` calls) + `DiscussionModelForum.vue`, `DiscussionModelComment.vue`, `DiscussionItem.vue` (all `inject` callers) | **No injection keys** - all 10 provide/inject pairs use bare string literals (`'viewMode'`, `'canBypassLock'`, `'setReplyToComment'`, `'toggleOfftopic'`, `'setQuoteOfComment'`, `'delete-comment'`, `'discussion'`, `'discussion-settings'`, `'thread-nsfw-revealed'`, `'canMarkOfftopic'`). Several call sites use `inject('x') as SomeType` casts with no default, meaning they silently return `undefined` if called outside the provider. Should be replaced with typed `InjectionKey<T>` symbols exported from a `Discussion.keys.ts` file. | High |
| 145 | `pages/forum/index.vue` (`provide('forumTopics', ...)`, `provide('forumActiveTopicId', ...)`) + `components/Forum/ForumModalAddDiscussion.vue` (`inject`) | Same bare-string pattern as #135. `ForumModalAddDiscussion.vue` has a fallback query when the injected value is missing but it is silently used - the typing gives no indication the inject can fail. | Medium |
| 146 | `pages/forum/[id].vue` (`provide('thread-nsfw-revealed', nsfwRevealed)`) | Shares the `'thread-nsfw-revealed'` key string with `Discussion.vue`'s own provide (item #135), but from a different provider. This means two different ancestors can provide the same key - whichever is closer wins. The string duplication across two files is the root cause; typed keys would make the collision obvious at compile time. | Medium |
...
| 148 | `components/Admin/Users/UserDetails.vue` lines 402, 405, 410 | `(user as any).website` ×3 | `website` is a column on `profiles` but the `user` prop here appears to be typed as a stricter subset that omits it. The correct fix is to widen the prop type to include `website: string \| null` rather than casting. Appears in the template so it also suppresses template type-checking entirely for that expression. | Medium |
| 149 | `components/Profile/ProfileHeader.vue` lines 338, 341, 346 | `(profile as any).website` ×3 | Identical to #139 in a different component - same `website` column missing from the prop type. Both #139 and #140 should be fixed together by adding `website` to the shared profile query select and prop interface. | Medium |
| 150 | `components/Forum/ForumDiscussionItem.vue` line 89, `components/Forum/ForumTopicItem.vue` line 88 | `$event as any` passed to `emit('update', ...)` | `ForumItemActions` emits a typed update event but the parent re-emits it with `as any`, losing the type at the boundary. The emit type in the parent should be widened to match `ForumItemActions`'s emit signature rather than casting. | Low |
| 151 | `components/Events/EventsCalendar.vue` line 320 | `:attributes="calendarAttributes as any"` | The VUI/v-calendar `attributes` prop type is not matching the locally computed shape. Should be typed with the library's exported `AttributeConfig` (or equivalent) rather than silenced with `as any`. | Low |
...
| # | File(s) | Issue | Priority |
|---|---------|-------|----------|
| 152 | `composables/useCacheBadgeDiscussionReplyCount.ts`, `useCacheBadgeDiscussionStartedCount.ts`, `useCacheBadgePartyAnimalCount.ts` | **Inconsistent return shape** - `useCacheBadgeDiscussionReplyCount` and `useCacheBadgePartyAnimalCount` return `{ count, loading, error, refresh }`. `useCacheBadgeDiscussionStartedCount` also returns `invalidate` as a fourth member. `useCacheUserDiscussionCount` returns the same four but with a different internal query scope. The inconsistency means callers can't be swapped to the generic factory (item #52) without auditing each call site. Fix is to standardise all four to `{ count, loading, error, refresh, invalidate }` before or as part of merging them. | High |
| 153 | `composables/useCacheUserDiscussionCount.ts` | **Semantic divergence from badge composables** - this composable counts *all* non-draft, non-profile discussions a user has created (used for a display counter in `DiscussionModelForum.vue`), while `useCacheBadgeDiscussionStartedCount` counts discussions in a specific topic context (used for badge threshold logic). They have the same scaffold but different query predicates. They should not be merged but should be clearly documented with the distinction - currently both have similarly vague docstrings. | Medium |
| 154 | `composables/useCacheGameAssets.ts` | **localStorage used directly** - this composable is the only cache composable that bypasses `useCache` and writes to `window.localStorage` directly with hand-rolled JSON serialization, TTL checking, and `try/catch` suppression. All other asset/data composables use the `useCache` in-memory store. The inconsistency means game asset cache entries survive page reloads (possibly desirable) but are invisible to `cache.invalidateByPattern` and `cache.clearCache`, and are never cleaned up by the shared TTL sweep. Should either adopt `useCache` like its siblings, or the localStorage approach should be extracted to a `usePersistentCache` composable and used consistently wherever persistence is wanted. | Medium |
| 155 | `composables/useCache.ts` | **`useCacheQuery` is nearly dead** - the `useCacheQuery` export (lines 314-430, ~116 lines) is a full auto-fetching Supabase query wrapper with filter/order/limit builder, but it is only called in 5 places (`UserDetails.vue`, `ProfileDetail.vue`, `UserPreviewCard.vue`, `votes/index.vue`, `votes/[id].vue`) and `auth/confirm.vue` uses the bare `useCache` directly. The `useCacheQuery` abstraction builds Supabase queries from a plain-object config, duplicating what PostgREST's chainable API already does fluently. Callers should evaluate whether direct Supabase queries with `useCache.set/get` (the pattern used by `useCacheUserData`) are cleaner, and `useCacheQuery` should be removed if all callers migrate. | Medium |
...
|---|---------------|----------------------|---------|----------|
| 158 | `pages/admin/index.vue` (dashboard) | `monthly_funding` ×3, `events` ×2, `complaints` ×2 | `KPIOverview` fetches `monthly_funding`, `events`, `complaints`, and `expenses` independently. `IncomeChart` fetches `monthly_funding` again. `UserChart` fetches `monthly_funding` a third time. `Alerts` fetches `complaints` and `events` again. All four fire on the same page mount with no shared cache and no coordination - the dashboard fires at least 8 Supabase queries where 5 would cover everything. Introduce a `useAdminDashboardData()` composable (or extend `useCache`) to share the `monthly_funding`, `complaints`, and `events` results across all four consumers. | High |
| 159 | `pages/community/funding.vue` | `monthly_funding` ×3 | `funding.vue` fetches all `monthly_funding` rows for `FundingHistory`, then renders `<FundingProgress>` which independently fetches the latest row, and `<SupportCTA>` (when rendered without sufficient props) fetches the latest row a third time. A `useFundingData()` composable owning the fetch and sharing it via `useCache` or `provide/inject` would eliminate the two redundant requests. (Also listed as item #67 - this entry adds the caching framing.) | High |
| 160 | `pages/admin/funding.vue` | `monthly_funding` ×3 | `FundingKPIs` issues two sequential `monthly_funding` queries (one for current month, one for previous month comparison) and `FundingTable` issues a third. The two KPI queries could be merged into a single `.limit(2)` call, and the result shared with `FundingTable` via a composable rather than re-fetched. | Medium |

### Components that fetch on every mount with no cache

| # | Component | Fetches | Issue | Priority |
|---|-----------|---------|-------|----------|
| 161 | `components/Layout/NavEventBadge.vue` | `events` (date, duration_minutes) on every page navigation | The component is embedded in `Navigation.vue` and re-mounts on every client-side navigation, firing a fresh `events` query each time with no TTL or cache. The events list changes at most a few times a day. Wrap the fetch in `useCache` with a 5-minute TTL, or share the already-fetched events list from `pages/events/index.vue` and `pages/index.vue` via a `useEvents()` composable. | High |
| 162 | `composables/useUserSettings.ts` | `settings` table - once per component that calls `useUserSettings()` | `fetchSettings` is called from `onBeforeMount` inside the composable body, meaning **every** component that calls `useUserSettings()` fires a DB query on mount. With `ThemeToggle` (rendered in the nav on every page), `Discussion.vue`, `pages/forum/index.vue`, and `pages/forum/[id].vue` all calling it, a forum thread page fires the settings query up to 3 times. The composable already uses `useState` to share the result reactively - it just needs a `hasFetched` guard (a simple `ref(false)` flipped after the first successful fetch) to skip redundant DB calls when the state is already populated. | High |
| 163 | `components/Shared/SupportModal.vue` | `user_roles` + `profiles` on mount | The modal is always mounted (no `v-if`) on `pages/servers/gameservers/index.vue` and `pages/auth/sign-in.vue`, so `loadAdmins()` fires two queries every time those pages load - even though the modal is almost never opened. Either add a `v-if="supportModalOpen"` to mount it lazily, or guard `loadAdmins()` to only run when the modal is first opened (`watch(open, ..., { once: true })`). | Medium |
| 164 | `components/Profile/ProfileDiscussions.vue` | `forum_discussion_replies` + `discussions` on every profile page visit | Two sequential queries with no caching fetch a user's recent activity on every profile view. The data is a good candidate for `useCache` with a short TTL (2-3 minutes), keyed by `profile-discussions:${profileId}`. | Medium |
| 165 | `components/Landing/LandingMotd.vue` | `motds` batch on every landing page mount | MOTDs are editorial content that changes infrequently (days/weeks). The component fetches a random batch on every mount with no cache. A `useCache` entry with a 10-minute TTL keyed by `motds:batch` would eliminate repeated fetches for users who navigate back to the landing page. | Low |
| 166 | `components/Shared/GameServerLink.vue` | `gameservers` (name only) per unique `gameserverId` prop | The component fetches the gameserver name on mount with no caching. It appears in `ComplaintsViewer.vue` which can render many complaint rows, potentially firing one query per row. Cache the name lookup in `useCache` keyed by `gameserver:name:${id}` with a long TTL (30 minutes - gameserver names rarely change). | Low |

### Uncached guard queries that are immediately superseded

| # | Location | Issue | Priority |
|---|----------|-------|----------|
| 167 | `pages/profile/[id].vue` | The page fires a `profiles.select('public')` query to decide whether to gate access, then `ProfileDetail` fires a full `profiles.select('*')` via `useCacheQuery` immediately after. On first visit the cache is cold so both queries hit the database. The guard result (`public: boolean`) is discarded after the redirect check. The guard should either use the `useCacheQuery` result (passed down as a prop) or be folded into the full profile fetch by selecting `public` as part of `ProfileDetail`'s own query and gating the render there. | Medium |

### Missing `useEvents()` composable - events fetched in 3 independent places

| # | Files | Issue | Priority |
|---|-------|-------|----------|
| 168 | `pages/index.vue`, `pages/events/index.vue`, `components/Layout/NavEventBadge.vue` | All three independently query the `events` table with no shared cache. `pages/index.vue` fetches upcoming events for the landing page cards; `pages/events/index.vue` fetches all events for the calendar/listing; `NavEventBadge` fetches the last 25 events for its countdown badge - and re-fetches on every navigation. A `useEvents()` composable backed by `useCache` (TTL 5 minutes) would serve all three and make the `NavEventBadge` tick without hitting the DB on every page transition. | Medium |
...
|---|-------|---------------------|-------------------|----------|
| 169 | `games` | `pages/servers/gameservers/index.vue` (select `*`), `pages/servers/gameservers/[id].vue` (single by ID), `pages/events/[id].vue` (by IDs array), `components/Shared/GameDetailsModal.vue` (single by ID), `components/Admin/Events/EventDetails.vue`, `EventForm.vue`, `GameServerForm.vue` - 7 uncached call sites with no coordination | `useGames()` - cache all games keyed `games:all` (TTL 20 min). Single-game lookups derive from the cached list. Admin write paths (`GameTable.vue`) call `invalidate()` after mutations. `useCacheGameAssets` already caches the storage URLs per-game; this covers the DB rows. | High |
| 170 | `discussion_topics` | `pages/forum/index.vue` (with full discussion tree), `components/Forum/ForumModalAddDiscussion.vue` (plain list, fallback), `components/Admin/Discussions/DiscussionDetails.vue` (plain list for picker) - 3 uncached call sites | `useForumTopics()` - cache the plain topic list keyed `discussion_topics:all` (TTL 30 min, topics almost never change). The full tree fetch in `forum/index.vue` is user-data-dependent and should stay separate; this covers the lightweight picker/fallback uses. | Medium |
| 171 | `motds` | `components/Landing/LandingMotd.vue` fetches a random batch on every landing page mount with no caching | Wrap `LandingMotd`'s `fetchBatch()` in `useCache` keyed `motds:batch:${offset}` (TTL 10 min). MOTDs change on the scale of days; re-fetching on every landing page visit is unnecessary. | Low |
...
|---|-------|-------|---------------|----------|
| 192 | `public.settings` | `anon` role granted all DML privileges including `DELETE` and `TRUNCATE`; RLS saves it in practice but the grant is incorrect | `REVOKE ALL ON public.settings FROM anon; GRANT SELECT ON public.settings TO anon;` (anon can't read rows due to RLS but the grant level is conventional). | Low |
